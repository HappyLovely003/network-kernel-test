#!/bin/bash
# [default] 큐 2 — CONFIG=C-1. 클라이언트 VM에서 실행. test/common.sh 상수 사용.
set -e
...
NIC="${VM_ETH:-enp2s0}"
mkdir -p "${OUTDIR}"
echo "[default/queue2] CONFIG=${CONFIG} NIC=${NIC} 큐 2"
...
echo "[default/queue2] 결과: ${OUTDIR}/ (05_summarize: result/날짜_시간/${CONFIG}/run1/ 로 복사)"
