#!/bin/bash
# [tuned] 큐 2 — CONFIG=D-1. test/common.sh 상수 사용.
set -e
...
NIC="${VM_ETH:-enp2s0}"
mkdir -p "${OUTDIR}"
echo "[tuned/queue2] CONFIG=${CONFIG} NIC=${NIC} 큐 2"
...
echo "[tuned/queue2] 결과: ${OUTDIR}/ (05_summarize: result/날짜_시간/${CONFIG}/run1/ 로 복사)"
